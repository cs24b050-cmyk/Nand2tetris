...
string writeCall(const string &functionName, int nArgs, int &Fcounter) {
    ...
    asmCode += "@SP\nD=M\n@5\nD=D-A\n@" + to_string(nArgs) + "\nD=D-A\n@ARG\nM=D\n"; // setting the latest arg = SP-n-5
    asmCode += "@SP\nD=M\n@LCL\nM=D\n"; // latest local points to stack top
    asmCode += "@" + functionName + "\n0;JMP\n"; // goto function label
    asmCode += "(" + returnLabel + ")\n"; // create a return label and to have unique label
...
int main(int argc, char *argv[]) {
    ...
    vector<string> vmFiles; // vector to store VM files
    ...
    while (SIZE < vmFiles.size()) {
        ...
        while (getline(fin, line)) {
            ...
            string arg1 = "", arg2 = "";
            // arg1 is for segment kind and arg2 is for index kind
            if (pos_s != string::npos) {
                // It is not a arithmetic command
                size_t pos_s2 = line.find(' ', pos_s + 1);
                if (pos_s2 != string::npos) {
                    // It may be a functional or push/pop seg index
